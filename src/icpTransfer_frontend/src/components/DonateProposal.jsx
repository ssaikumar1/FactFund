...
const DonateProposal = ({ actor, notify, principal }) => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const prop = {
            ...
            amount_raised: val.amount_raised ? Number(val.amount_raised) / 10 ** 8 : 0, // Add default of 0
  ...
  const handleDonateClick = async () => {
    ...
    try {
      ...
      if (result.height) {
        notify("Transfer successful! Syncing transaction...")
        setIsSyncing(true)

        try {
          const syncResult = await icpTransfer_backend.syncTransactions(Number(id))

          if (syncResult.ok) {
            notify("Syncing completed successfully!")
            const val = syncResult.ok
            const b64 = "data:image/webp;base64," + (await bufferToBase64(val.image))
            ...
            const updatedProposal = {
              ...
              created_by_text: Principal.from(val.created_by).toString(),
              amount_required: Number(val.amount_required) / 10 ** 8,
              amount_raised: Number(val.amount_raised) / 10 ** 8,
            ...
            notify(`Syncing failed: ${syncResult.err || "Unknown error"}`)
          }
        } catch (syncError) {
          console.error("Sync error:", syncError)
          notify(`Syncing error: ${syncError.message || "Failed to sync transaction"}`)
        } finally {
          setIsSyncing(false)
        }
      }
  ...
  const handleClaimClick = async () => {
    ...
    try {
      ...
      const recipientPrincipal = Principal.fromText(claimPrincipal)

      const result = await actor.claimProposal(Number(id), recipientPrincipal)

      if (result.ok) {
        notify(`Successfully sent funds to ${recipientPrincipal.toString()}`)
